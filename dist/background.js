...
function accessRequstHeaders(details) {
    console.log("Request details: ", details);
    return { requestHeaders: details.requestHeaders };
}
browser.webRequest.onBeforeSendHeaders.addListener(accessRequstHeaders, { urls: [filter.urls[0], filter.urls[1]] }, ["blocking", "requestHeaders"]);
browser.tabs.onUpdated.addListener((id, changeInfo, tab) => {
    // console.log("tab updated event", tab.id);
    browser.pageAction.show(tab.id);
}, filter);
async function downloadCSV(surveyId, server, csvType = 'csv') {
    ...
    try {
        ...
        let requestBody = await getRP(surveyId, server);
...
async function getRP(surveyId, server) {
    ...
    try {
        ...
        if (!csrfToken)
            throw new Error("Invalid or CSRF token not found");
